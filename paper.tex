...
A Lindbladian is \emph{primitive} (or irreducible) if its commutant is trivial: $\text{Comm}(\Lap) = \mathbb{C} \cdot I$.
\end{definition}

\begin{theorem}[Primitivity Equivalence]
The following are equivalent:
\begin{enumerate}[label=(\roman*)]
    \item $\Lap$ is primitive
    \item $\delta_Q = 0$
...
\begin{theorem}[Ergodic Peripheral Spectrum]
\label{thm:ergodic-peripheral}
For ergodic systems ($\delta_Q = 0$), the peripheral spectrum is $\{0\}$ (no oscillations).
\end{theorem}

\begin{conjecture}[Phase Group Structure]
The peripheral phases form a finitely generated abelian group $\mathbb{Z}^k$ for some $k \geq 0$. The generators correspond to fundamental frequencies of the dynamics.
\end{conjecture}

\subsection{Classification Conjecture}

\begin{conjecture}[Complete Classification]
Two Lindbladians $\Lap_1, \Lap_2$ with faithful stationary states are equivalent iff:
\begin{enumerate}[label=(\roman*)]
...
\end{enumerate}
Note: $\delta_{\mathrm{cen}}$ is redundant (equals number of Wedderburn blocks minus 1).
\end{conjecture}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item $\rho^*$ is globally attracting
\end{enumerate}
...
\midrule
Thm.~\ref{thm:classification} & \texttt{quantum\_deficiency\_eq\_central\_deficiency} & Quantum/Classification.lean \\
