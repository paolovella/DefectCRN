...
\textbf{This constitutes the first machine-checked formalization of comprehensive chemical reaction network theory, including its quantum generalization.} The classical framework---comprising \textbf{7930 lines} of Lean 4 code with \textbf{255 theorems} and \textbf{zero axioms}---has been formally verified using the Mathlib library. The quantum extension adds \textbf{7680 lines} with \textbf{339 theorems} and \textbf{22 well-documented axioms}, including the \textbf{universal classification theorem} $\delta_Q = \delta_{\mathrm{com}}$ connecting dynamical and algebraic invariants under faithful stationary states, and the \textbf{characterization} $\delta_Q = \delta_{\mathrm{cen}} \Leftrightarrow \text{multiplicity-free}$. We demonstrate the theory on examples ranging from the simple $n$-cycle to the complete TCA (Krebs) cycle with 16 species and 10 reactions.
\end{abstract}
...
By ``formally verified,'' we mean that every definition, theorem, and proof has been machine-checked in the Lean 4 proof assistant, yielding \textbf{15,623 total lines} of code with \textbf{573 theorems} across 43 files. The classical CRNT portion is fully axiom-free; the quantum extension uses 20 well-documented axioms for deep results requiring additional Mathlib infrastructure (Wedderburn structure, Evans--H\o egh-Krohn theorem, spectral analysis).
...
By the Evans--H\o egh-Krohn theorem, $\dim(\mathrm{commutant}) = \dim(\mathrm{stationary})$ under a faithful stationary state. The commutant of the interaction algebra $A_{\mathrm{int}}'$ is the algebraic version of the GKSL commutant. Thus $\dim(A_{\mathrm{int}}') = \dim(\ker \Lap)$, giving $\delta_{\mathrm{com}} = \delta_Q$.
\end{proof}
...
Files & 28 & 15 \\
\midrule
\textbf{Total} & \multicolumn{2}{r}{\textbf{15,623 lines, 573 theorems, 43 files}} \\
\bottomrule
\end{tabular}
\end{center}
