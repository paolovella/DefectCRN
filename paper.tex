...
\textbf{This constitutes the first machine-checked formalization of comprehensive chemical reaction network theory, including its quantum generalization.} The classical framework---comprising \textbf{7930 lines} of Lean 4 code with \textbf{255 theorems} and \textbf{zero axioms}---has been formally verified using the Mathlib library. The quantum extension adds \textbf{7680 lines} with \textbf{339 theorems} and \textbf{22 well-documented axioms}, including the \textbf{main classification theorem} $\delta_Q = \delta_{\mathrm{cen}}$ connecting dynamical and algebraic invariants under faithful stationary states. We demonstrate the theory on examples ranging from the simple $n$-cycle to the complete TCA (Krebs) cycle with 16 species and 10 reactions.
\end{abstract}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{Quantum CRNT} (Section \ref{sec:quantum}): Lindblad dynamics, interaction algebra, Wedderburn decomposition, structural deficiency, \textbf{main classification theorem} $\delta_Q = \delta_{\mathrm{cen}}$, deficiency hierarchy, quantum deficiency zero theorem.
...
The central result connecting quantum dynamics to algebraic structure is:

\begin{theorem}[Classification Theorem]
\label{thm:classification}
Under a faithful stationary state, the quantum deficiency equals the central deficiency:
\[
\delta_Q = \delta_{\mathrm{cen}}
\]
where $\delta_{\mathrm{cen}} = \dim(Z(A_{\mathrm{int}})) - 1$ is the dimension of the center of the interaction algebra minus one.
\end{theorem}

\begin{proof}[Proof sketch]
The proof proceeds in three steps:
\begin{enumerate}
    \item By Evans--H\o egh-Krohn, $\dim(\mathrm{Comm}) = \dim(\mathrm{stationary})$ under faithful state
    \item Interaction algebras are multiplicity-free (all $m_\alpha = 1$ in Wedderburn decomposition), so $\dim(\mathrm{center}) = \dim(\mathrm{commutant})$
    \item Combining: $\dim(\mathrm{center}) = \dim(\mathrm{stationary})$, hence $\delta_{\mathrm{cen}} = \delta_Q$
\end{enumerate}
\end{proof}
...
\[
\delta_{\mathrm{struct}} \leq \delta_{\mathrm{cen}} = \delta_Q
\]
where $\delta_{\mathrm{struct}}$ is the structural deficiency from the quantum network graph.
\end{theorem}

\begin{definition}[Deficiency Gap]
The \emph{deficiency gap} $\delta_{\mathrm{cen}} - \delta_{\mathrm{struct}}$ measures ``accidental'' symmetry---block structure in the interaction algebra not visible from the graph alone.
\end{definition}

\begin{theorem}[Gap Characterization]
\label{thm:gap}
The gap is zero iff $\delta_{\mathrm{struct}} = \delta_{\mathrm{cen}}$, which occurs when the graph perfectly captures the algebra's block structure. A positive gap indicates hidden symmetry.
\end{theorem}
...
--- & Hierarchy: $\delta_{\mathrm{struct}} \leq \delta_{\mathrm{cen}} = \delta_Q$ \\
\bottomrule
\end{tabular}
\end{center}
\end{remark}
...
\begin{enumerate}
    ...
    \item \textbf{Quantum}: Lindblad dynamics extends CRNT to open quantum systems with $\delta_Q = \delta_{\mathrm{cen}}$ classification theorem and deficiency hierarchy $\delta_{\mathrm{struct}} \leq \delta_{\mathrm{cen}} = \delta_Q$
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \texttt{interactionAlgebra\_multiplicityFree}: Representation theory (multiplicity-free)
...
\texttt{Quantum/Classification.lean} & Main theorem $\delta_Q = \delta_{\mathrm{cen}}$ & 336 & 15 \\
