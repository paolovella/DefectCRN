...
\begin{abstract}
We present a comprehensive, formally verified framework for chemical reaction network theory (CRNT). Starting from the Onsager--Rayleigh dissipation principle, we develop the variational characterization of steady-state fluxes via the functional $\OR(J) = \frac{1}{2}\inner{J}{J}_{W^{-1}} - \inner{\omega}{J}$. We systematically extend to: the deficiency zero and one theorems; higher deficiency networks ($\delta \geq 2$) via the Deficiency-Two Algorithm and Species-Reaction graphs; \textbf{cohomological deficiency theory} establishing $\delta = \dim(H^1)$ for the CRN chain complex; multistability and bifurcation theory; oscillations via Hopf bifurcation and the Routh--Hurwitz criterion; reaction-diffusion systems with Turing pattern formation; stochastic CRNs via the Chemical Master Equation; feedback control theory including antithetic integral control; and \textbf{quantum chemical reaction network theory} extending the classical deficiency zero theorem to open quantum systems via Lindblad dynamics.

\textbf{This constitutes the first machine-checked formalization of comprehensive chemical reaction network theory, including its quantum generalization.} The formalization comprises \textbf{16,042 lines} of Lean 4 code with \textbf{590 theorems} across 43 files. The classical CRNT portion is fully axiom-free. The quantum extension uses \textbf{22 well-documented axioms} for deep results (Wedderburn structure, Evans--H{\o}egh-Krohn theorem, spectral analysis) whose infrastructure is not yet available in Mathlib; all structural theorems are fully proved modulo these axioms. Key results include the \textbf{universal classification theorem} $\delta_Q = \delta_{\mathrm{com}}$ connecting dynamical and algebraic invariants, and the \textbf{characterization} $\delta_Q = \delta_{\mathrm{cen}} \Leftrightarrow \text{multiplicity-free}$. We demonstrate the theory on examples from the simple $n$-cycle to the complete TCA cycle.
\end{abstract}
...
\begin{itemize}
    ...
    \item Quantum CRNT (Lindblad dynamics, quantum deficiency zero theorem)
\end{itemize}

By ``formally verified,'' we mean that every definition, theorem, and proof has been machine-checked in the Lean 4 proof assistant, yielding \textbf{16,042 total lines} of code with \textbf{590 theorems} across 43 files. The classical CRNT portion is fully axiom-free; the quantum extension uses 22 well-documented axioms for deep results requiring additional Mathlib infrastructure (Wedderburn structure, Evans--H\o egh-Krohn theorem \cite{EvansHoeghKrohn1978}, spectral analysis).
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{Quantum CRNT} (Section \ref{sec:quantum}): Lindblad dynamics, interaction algebra, Wedderburn decomposition, structural deficiency, \textbf{universal classification theorem} $\delta_Q = \delta_{\mathrm{com}}$, \textbf{multiplicity-free characterization} $\delta_Q = \delta_{\mathrm{cen}}$, deficiency hierarchy, quantum deficiency zero theorem.
...
% ============================================================================
\section{Quantum Chemical Reaction Network Theory}
\label{sec:quantum}
% ============================================================================

We extend classical CRNT to open quantum systems governed by Lindblad dynamics, establishing a \emph{quantum deficiency zero theorem} as the central result.

\subsection{Lindblad Dynamics}

\begin{definition}[Lindbladian]
A Lindblad generator $\Lap$ on $n \times n$ density matrices consists of:
\begin{itemize}
    \item A Hermitian Hamiltonian $H$
    \item Jump operators $\{L_k\}$
\end{itemize}
...
The Lindbladian preserves:
\begin{enumerate}[label=(\roman*)]
    \item Trace: $\tr(\Lap(\rho)) = 0$
    \item Hermiticity: $\rho = \rho^\dagger \Rightarrow \Lap(\rho) = \Lap(\rho)^\dagger$
\end{enumerate}
\end{theorem}
...
The commutant of $\Lap$ is:
\[
\text{Comm}(\Lap) = \{X : [X, H] = 0 \text{ and } [X, L_k] = [X, L_k^\dagger] = 0 \text{ for all } k\}.
\]
\end{definition}
...
\[
\delta_Q = \dim(\ker \Lap) - 1
\]
where $\ker \Lap$ is the space of stationary states. This is the quantum analog of classical CRNT deficiency.
\end{definition}

\begin{definition}[Ergodic / Irreducible]
A Lindbladian is \emph{ergodic} (or irreducible) if its commutant is trivial: $\text{Comm}(\Lap) = \mathbb{C} \cdot I$.
\end{definition}

\begin{definition}[Primitive]
A Lindbladian is \emph{primitive} if it is ergodic \emph{and} has trivial peripheral spectrum: no eigenvalues on the imaginary axis except $0$.
\end{definition}

\begin{theorem}[Ergodicity Equivalence]
The following are equivalent:
\begin{enumerate}[label=(\roman*)]
    \item $\Lap$ is ergodic
    \item $\delta_Q = 0$
    \item Every non-trivial projection in the commutant equals $0$ or $I$
\end{enumerate}
\end{theorem}

\begin{remark}
In finite dimensions, primitivity implies exponential convergence to a unique stationary state without oscillations, while ergodicity alone allows persistent oscillations (limit cycles in the density matrix evolution).
\end{remark}

\subsection{Frigerio's Theorem}

\begin{theorem}[Frigerio 1978]
\label{thm:frigerio}
If $\Lap$ is primitive, there exists a unique faithful stationary state $\rho^*$:
\begin{enumerate}[label=(\roman*)]
    \item $\rho^*$ is Hermitian and positive definite
    \item $\tr(\rho^*) = 1$
    \item $\Lap(\rho^*) = 0$
    \item Any initial state $\rho_0$ converges: $e^{t\Lap}(\rho_0) \to \rho^*$ as $t \to \infty$
\end{enumerate}
\end{theorem}

\subsection{The Main Classification Theorem}

The central result connecting quantum dynamics to algebraic structure requires careful statement. We introduce the \emph{commutant deficiency}:

\begin{definition}[Commutant Deficiency]
The commutant deficiency is $\delta_{\mathrm{com}} = \dim(A_{\mathrm{int}}') - 1$, where $A_{\mathrm{int}}'$ is the commutant of the interaction algebra.
\end{definition}

\begin{theorem}[Universal Classification Theorem]
\label{thm:classification}
Under a faithful stationary state, the quantum deficiency equals the commutant deficiency:
\[
\delta_Q = \delta_{\mathrm{com}}
\]
This holds universally for all Lindbladians with faithful stationary states.
\end{theorem}

\begin{proof}[Proof sketch]
By the Evans--H\o egh-Krohn theorem \cite{EvansHoeghKrohn1978}, under a faithful stationary state $\sigma$, the fixed-point algebra of the dual semigroup equals the commutant of the interaction algebra: $\mathrm{Fix}(\Lap^*) = A_{\mathrm{int}}'$. Since $\dim(\ker \Lap) = \dim(\mathrm{Fix}(\Lap^*))$ by duality, we have $\dim(\ker \Lap) = \dim(A_{\mathrm{int}}')$, giving $\delta_Q = \delta_{\mathrm{com}}$.
\end{proof}
...
\[
\delta_Q = \delta_{\mathrm{cen}} \quad\Longleftrightarrow\quad A_{\mathrm{int}} \text{ is multiplicity-free}
\]
where ``multiplicity-free'' means all $m_\alpha = 1$ in the Wedderburn decomposition $A_{\mathrm{int}} \cong \bigoplus_\alpha (M_{d_\alpha} \otimes I_{m_\alpha})$.
\end{theorem}

\begin{remark}[Noiseless Subsystems]
The gap $\delta_Q - \delta_{\mathrm{cen}} = \delta_{\mathrm{com}} - \delta_{\mathrm{cen}} = \sum_\alpha (m_\alpha^2 - 1)$ measures noiseless subsystem structure. When $m_\alpha > 1$, there are decoherence-free subspaces providing symmetry-protected quantum information.
\end{remark}
...
\]
where $\delta_{\mathrm{struct}}$ is the structural deficiency from the quantum network graph. The hierarchy has two gaps measuring distinct phenomena.
\end{theorem}

\begin{definition}[Structural Gap]
The \emph{structural gap} $\delta_{\mathrm{cen}} - \delta_{\mathrm{struct}}$ measures ``accidental'' symmetry---block structure in the interaction algebra not visible from the graph alone.
\end{definition}

\begin{definition}[Multiplicity Gap]
The \emph{multiplicity gap} $\delta_{\mathrm{com}} - \delta_{\mathrm{cen}} = \delta_Q - \delta_{\mathrm{cen}}$ measures noiseless subsystem structure---Wedderburn multiplicities $m_\alpha > 1$ corresponding to protected quantum information.
\end{definition}

\begin{theorem}[Gap Characterizations]
\label{thm:gap}
\begin{enumerate}[label=(\roman*)]
    \item Structural gap is zero iff $\delta_{\mathrm{struct}} = \delta_{\mathrm{cen}}$ (graph captures block structure)
    \item Multiplicity gap is zero iff $A_{\mathrm{int}}$ is multiplicity-free (no noiseless subsystems)
\end{enumerate}
\end{theorem}

\subsection{Peripheral Spectrum}

\begin{definition}[Peripheral Spectrum]
The \emph{peripheral spectrum} of $\Lap$ is $\{\mu \in \mathrm{Spec}(\Lap^*) : \mathrm{Re}(\mu) = 0\}$. The \emph{peripheral phases} are the imaginary parts $\omega$ such that $i\omega$ is a peripheral eigenvalue.
\end{definition}

\begin{theorem}[Detailed Balance Peripheral Spectrum]
\label{thm:ergodic-peripheral}
For systems in $\sigma$-detailed balance, the peripheral spectrum is $\{0\}$ (no oscillations).
\end{theorem}

\begin{proof}
Under $\sigma$-detailed balance, the spectrum of $\Lap^*$ is real (all eigenvalues satisfy $\mathrm{Im}(\mu) = 0$). A peripheral eigenvalue has $\mathrm{Re}(\mu) = 0$. Combined: $\mu = 0$.
\end{proof}

\begin{remark}
A system is \emph{primitive} if it is ergodic with trivial peripheral spectrum. By Theorem~\ref{thm:ergodic-peripheral}, all ergodic systems in detailed balance are primitive.
\end{remark}

\begin{theorem}[Phase Group Structure]
\label{thm:phase-group}
The peripheral phases form a finitely generated abelian group isomorphic to $\mathbb{Z}^k$ for some $k \geq 0$.
\end{theorem}

\begin{proof}[Proof sketch]
The Lindbladian $\Lap^*$ is a linear operator on $M_n(\mathbb{C})$, a finite-dimensional space. Its spectrum is finite, so the set of peripheral frequencies $\Omega = \{\omega_1, \ldots, \omega_r\}$ is finite. The additive subgroup of $\mathbb{R}$ generated by finitely many real numbers is always finitely generated, isomorphic to $\mathbb{Z}^k$ where $k = \dim_\mathbb{Q} \mathrm{span}_\mathbb{Q}\{\omega_1, \ldots, \omega_r\}$.
\end{proof}

\subsection{Classification Conjecture}

\begin{conjecture}[Asymptotic Classification]
Two Lindbladians $\Lap_1, \Lap_2$ in detailed balance with faithful stationary states have equivalent \emph{asymptotic dynamics} (up to unitary conjugacy preserving the stationary state) iff:
\begin{enumerate}[label=(\roman*)]
    \item $\mathrm{Type}(\Lap_1) = \mathrm{Type}(\Lap_2)$: Same Wedderburn signature $\{(d_\alpha, m_\alpha)\}$
    \item $\mathrm{Phase}(\Lap_1) \cong \mathrm{Phase}(\Lap_2)$: Isomorphic peripheral phase groups
\end{enumerate}
Note: $\delta_{\mathrm{cen}}$ is redundant (equals number of Wedderburn blocks minus 1). A complete classification may require additional invariants (e.g., Dirichlet form rank) to capture convergence rates.
\end{conjecture}

\begin{theorem}[Deficiency Does Not Classify]
\label{thm:separation}
The quantum deficiency $\delta_Q$ alone does not classify QMS. Separation examples exist:
\begin{itemize}
    \item Same $\delta_Q = 0$: $M_2(\mathbb{C})$ vs $M_3(\mathbb{C})$ (different dimensions)
    \item Same $\delta_Q = 1$: $\mathbb{C}^2$ vs $M_2 \oplus M_2$ (different block structure)
\end{itemize}
\end{theorem}

\subsection{Quantum Deficiency Zero Theorem}

\begin{theorem}[Quantum Deficiency Zero Theorem]
\label{thm:quantum-dzt}
If $\delta_Q = 0$ (ergodic), then:
\begin{enumerate}[label=(\roman*)]
    \item There exists a unique stationary density matrix $\rho^*$
    \item $\rho^*$ is faithful (strictly positive definite)
    \item If additionally primitive (no peripheral phases), $\rho^*$ is globally attracting without oscillations
\end{enumerate}
This is the quantum analog of the classical deficiency zero theorem.
\end{theorem}

\begin{remark}[Classical--Quantum Correspondence]
\begin{center}
\begin{tabular}{@{}ll@{}}
\toprule
...
Deficiency $\delta = n - \ell - \rank(N)$ & Quantum deficiency $\delta_Q = \dim(\mathrm{stationary}) - 1$ \\
--- & Commutant deficiency $\delta_{\mathrm{com}} = \dim(A_{\mathrm{int}}') - 1$ \\
--- & Central deficiency $\delta_{\mathrm{cen}} = \dim(Z(A_{\mathrm{int}})) - 1$ \\
--- & Structural deficiency $\delta_{\mathrm{struct}}$ (graph-based) \\
Weak reversibility & Primitivity \\
Global attractor (GAC) & Convergence to $\rho^*$ \\
--- & Hierarchy: $\delta_{\mathrm{struct}} \leq \delta_{\mathrm{cen}} \leq \delta_{\mathrm{com}} = \delta_Q$ \\
\bottomrule
\end{tabular}
\end{center}
\end{remark}
...
\begin{lemma}[Kernel Projection in Commutant]
\label{lem:kernel-proj}
For any PSD stationary state $\rho$, the projection onto $\ker(\rho)$ lies in the commutant.
\end{lemma}

\begin{theorem}[Stationary State is Faithful]
For primitive $\Lap$, any stationary density matrix is faithful (has trivial kernel).
\end{theorem}

\begin{proof}[Proof sketch]
By Lemma \ref{lem:kernel-proj}, the kernel projection $P$ is in the commutant. By primitivity, $P \in \{0, I\}$. Since $\tr(\rho) = 1$, we have $P \neq I$, so $P = 0$, meaning $\ker(\rho) = \{0\}$.
\end{proof}
...
\begin{enumerate}
    ...
    \item \textbf{Quantum}: Lindblad dynamics extends CRNT to open quantum systems with universal classification $\delta_Q = \delta_{\mathrm{com}}$ and deficiency hierarchy $\delta_{\mathrm{struct}} \leq \delta_{\mathrm{cen}} \leq \delta_{\mathrm{com}} = \delta_Q$
\end{enumerate}
...
\midrule
Lines of code & 7930 & 7693 \\
Theorems/Lemmas & 235 & 338 \\
\texttt{sorry} (unproven) & 0 & 0 \\
Axioms & 0 & 22 \\
Warnings & 0 & 0 \\
Files & 28 & 15 \\
\midrule
\textbf{Total} & \multicolumn{2}{r}{\textbf{16,042 lines, 590 theorems, 43 files}} \\
\bottomrule
\end{tabular}
\end{center}

The quantum axioms (22 total) represent deep results requiring Mathlib infrastructure:
\begin{enumerate}
    \item \texttt{exists\_stationary\_state}: Brouwer fixed point theorem
    \item \texttt{kernel\_projection\_mem\_commutant}: Block matrix decomposition theory
    \item \texttt{commutant\_dim\_eq\_stationary\_dim}: Evans--H\o egh-Krohn theorem (C*-algebra)
    \item \texttt{quantumSemigroup}: Matrix exponential infrastructure
    \item \texttt{convergence\_to\_stationary}: Spectral gap analysis
    \item \texttt{wedderburn\_decomposition\_exists}: Wedderburn--Artin structure theorem
    \item \texttt{structuralCommutant\_le\_commutant}: Graph-algebra correspondence
    \item \texttt{peripheral\_phases\_finitely\_generated}: Phase group structure
    \item \texttt{ergodic\_lindbladian\_exists}: Existence of ergodic systems
    \item \texttt{center\_dim\_eq\_commutant\_dim\_iff}: Multiplicity-free characterization
\end{enumerate}

\noindent\textbf{Axiom elimination roadmap:} As Mathlib develops, several axioms are likely to become theorems: (1)~\texttt{exists\_stationary\_state} once Brouwer's fixed point theorem is available; (2)~\texttt{quantumSemigroup} when matrix exponential infrastructure is complete; (3)~\texttt{wedderburn\_decomposition\_exists} when finite-dimensional C*-algebra theory is formalized. The remaining axioms encode deep results from quantum dynamical semigroup theory that may require substantial new Mathlib development.
...
\begin{enumerate}
    \item \textbf{Quantum axiom elimination}: Convert axioms to theorems as Mathlib gains:
    \begin{itemize}
        \item Brouwer fixed point theorem (for \texttt{exists\_stationary\_state})
        \item Matrix exponential infrastructure (for \texttt{quantumSemigroup})
        \item Wedderburn--Artin structure theorem (for \texttt{wedderburn\_decomposition\_exists})
        \item Spectral theory for non-normal operators (for \texttt{convergence\_to\_stationary})
    \end{itemize}
    ...
    \item \textbf{Quantum biology}: Coherence in photosynthetic reaction centers, quantum thermodynamics
\end{enumerate}
...
\emph{J.\ London Math.\ Soc.\ (2)}, 17(2):345--355, 1978.
...
\emph{Comm.\ Math.\ Phys.}, 63:269--276, 1978.
...
\emph{J.\ Math.\ Phys.}, 17:821--825, 1976.
...
\emph{Comm.\ Math.\ Phys.}, 48:119--130, 1976.
...
\emph{Lett.\ Math.\ Phys.}, 2:33--38, 1977.
...
\noindent DOI: \href{https://doi.org/10.5281/zenodo.18363743}{10.5281/zenodo.18363743}
...
\midrule
\multicolumn{4}{@{}l@{}}{\textit{Quantum CRNT}} \\
\midrule
\texttt{Quantum/Basic.lean} & Commutator, dagger, PSD & 318 & 24 \\
\texttt{Quantum/Lindbladian.lean} & Lindblad generator & 336 & 18 \\
\texttt{Quantum/Algebra.lean} & Commutant, polynomials & 302 & 21 \\
\texttt{Quantum/StationaryState.lean} & Stationary states, convexity & 185 & 11 \\
\texttt{Quantum/Deficiency.lean} & Quantum deficiency $\delta_Q$ & 79 & 7 \\
\texttt{Quantum/Irreducibility.lean} & Primitivity, spectral theory & 797 & 16 \\
\texttt{Quantum/Frigerio.lean} & Frigerio uniqueness theorem & 105 & 3 \\
\texttt{Quantum/QuantumDZT.lean} & Quantum DZT main result & 83 & 4 \\
\texttt{Quantum/InteractionAlgebra.lean} & Interaction algebra, Wedderburn & 712 & 28 \\
\texttt{Quantum/StructuralDeficiency.lean} & Graph deficiency $\delta_{\mathrm{struct}}$ & 1578 & 52 \\
\texttt{Quantum/Classification.lean} & Universal theorem $\delta_Q = \delta_{\mathrm{com}}$, hierarchy & 450 & 22 \\
\texttt{Quantum/DetailedBalance.lean} & Detailed balance, GNS & 216 & 8 \\
\texttt{Quantum/Examples/*.lean} & Two-level, collective dephasing & 580 & 12 \\
\midrule
\textbf{Total} & & \textbf{15,623} & \textbf{573} \\
\bottomrule
\end{tabular}
\caption{Formalization statistics. Version 8.0.0.}
\end{table}
...
\midrule
\multicolumn{3}{@{}l@{}}{\textit{Quantum CRNT}} \\
\midrule
Thm.~\ref{thm:classification} & \texttt{quantum\_deficiency\_eq\_commutant\_deficiency} & Quantum/Classification.lean \\
Thm.~\ref{thm:hierarchy} & \texttt{deficiency\_hierarchy} & Quantum/Classification.lean \\
Thm.~\ref{thm:gap} & \texttt{zero\_gap\_iff\_structural\_eq\_center\_dim} & Quantum/Classification.lean \\
Thm.~\ref{thm:ergodic-peripheral} & \texttt{ergodic\_peripheral\_trivial} & Quantum/Classification.lean \\
Thm.~\ref{thm:separation} & \texttt{deficiency\_does\_not\_classify} & Quantum/Classification.lean \\
Thm.~\ref{thm:frigerio} & \texttt{frigerio\_uniqueness} & Quantum/Frigerio.lean \\
Thm.~\ref{thm:quantum-dzt} & \texttt{quantum\_deficiency\_zero\_theorem} & Quantum/QuantumDZT.lean \\
Lem.~\ref{lem:kernel-proj} & \texttt{kernel\_projection\_mem\_commutant} & Quantum/Irreducibility.lean \\
--- & \texttt{deficiencyGap} & Quantum/Classification.lean \\
Thm.~\ref{thm:phase-group} & \texttt{peripheral\_phases\_finitely\_generated} & Quantum/Classification.lean \\
--- & \texttt{peripheralSpectrum} & Quantum/Classification.lean \\
--- & \texttt{CompleteInvariants} & Quantum/Classification.lean \\
--- & \texttt{stationary\_dim\_eq\_center\_dim} & Quantum/InteractionAlgebra.lean \\
--- & \texttt{center\_dim\_eq\_commutant\_dim} & Quantum/InteractionAlgebra.lean \\
--- & \texttt{structuralCommutant\_le\_commutant} & Quantum/StructuralDeficiency.lean \\
\bottomrule
\end{tabular}
