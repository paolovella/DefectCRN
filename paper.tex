...
The classical framework---comprising \textbf{7926 lines} of Lean 4 code with \textbf{235 theorems} and \textbf{zero axioms}---has been formally verified using the Mathlib library. The quantum extension adds \textbf{2454 lines} with \textbf{102 theorems} and \textbf{5 well-documented axioms} representing deep results requiring additional Mathlib infrastructure (Brouwer fixed point, matrix exponential, spectral analysis). We demonstrate the theory on examples ranging from the simple $n$-cycle to the complete TCA (Krebs) cycle with 16 species and 10 reactions. This constitutes the first machine-checked formalization of comprehensive chemical reaction network theory, including its quantum generalization.
\end{abstract}
...
By ``formally verified,'' we mean that every definition, theorem, and proof has been machine-checked in the Lean 4 proof assistant, yielding 10,380 total lines of code with 337 theorems across 37 files. The classical CRNT portion is fully axiom-free; the quantum extension uses 5 well-documented axioms for deep results requiring additional Mathlib infrastructure.
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{Quantum CRNT} (Section \ref{sec:quantum}): Lindblad dynamics, quantum deficiency, primitivity, Frigerio's theorem, quantum deficiency zero theorem.
...
Deficiency $\delta = n - \ell - \rank(N)$ & Quantum deficiency $\delta_Q = \dim(\text{Comm}) - 1$ \\
...
\begin{enumerate}
    ...
    \item \textbf{Quantum}: Lindblad dynamics extends CRNT to open quantum systems with $\delta_Q = 0$ theorem
\end{enumerate}
...
\midrule
Lines of code & 7926 & 2454 \\
Theorems/Lemmas & 235 & 102 \\
\texttt{sorry} (unproven) & 0 & 0 \\
Axioms & 0 & 5 \\
Warnings & 0 & 0 \\
Files & 28 & 9 \\
\midrule
\textbf{Total} & \multicolumn{2}{r}{\textbf{10,380 lines, 337 theorems, 37 files}} \\
\bottomrule
\end{tabular}
\end{center}

The quantum axioms represent deep results requiring Mathlib infrastructure not yet available:
\begin{enumerate}
    ...
    \item \texttt{commutant\_dim\_eq\_stationary\_dim}: C*-algebra structure theory
...
\midrule
\multicolumn{4}{@{}l@{}}{\textit{Quantum CRNT (new in v6.0.0)}} \\
\midrule
...
\texttt{Quantum/Deficiency.lean} & Quantum deficiency $\delta_Q$ & 173 & 7 \\
...
\midrule
\textbf{Total} & & \textbf{10,380} & \textbf{337} \\
\bottomrule
\end{tabular}
\caption{Formalization statistics. Version 6.0.0.}
\end{table}
...
--- & \texttt{primitive\_iff\_irreducible} & Quantum/Irreducibility.lean \\
--- & \texttt{hermitian\_commutant\_is\_scalar} & Quantum/Irreducibility.lean \\
\bottomrule
\end{tabular}
