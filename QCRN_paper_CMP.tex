...
\item Coherent edges: $(i,j) \in E_{\mathrm{coh}}$ (symmetric) iff $H_{ij} \neq 0$
\item Jump edges: $(i \to j) \in E_{\mathrm{jump}}$ iff $\exists k$ with $(L_k)_{ji} \neq 0$
\end{itemize}
\end{definition}
...
$\Theta(G) = \{(H, \{L_k\}) : \mathrm{supp}(H) \subseteq E_{\mathrm{coh}}, \mathrm{supp}(L_k) \subseteq E_{\mathrm{jump}}\}$.
\end{definition}

\textbf{Remark.} The parameter space $\Theta(G)$ allows arbitrary complex entries on the support pattern (subject to $H = H^\dagger$). Physical constraints---such as particular Lindblad decompositions or positivity requirements on transition rates---may restrict to a proper subset. The genericity statement (Theorem~\ref{thm:robust}(b)) applies within $\Theta(G)$; for restricted parameter sets, one must verify that the restriction intersects the generic locus.

\begin{theorem}[Parameter Robustness]\label{thm:robust}
Let $G$ be a quantum network graph.
\begin{enumerate}
\item[(a)] For any $\theta \in \Theta(G)$: $\commutant(\theta) \supseteq \commutant_{\mathrm{struct}}(G)$, hence $\defQ(\theta) \leq \defQ^{\mathrm{struct}}(G)$.
...
\begin{proof}
(a) $\commutant_{\mathrm{struct}}(G)$ is the intersection over \emph{all} operators with support in $G$; $\commutant(\theta)$ is for specific $H, L_k, L_k^\dagger$.
...
($\Rightarrow$) If $D(G)$ is not strongly connected, let $S \subsetneq V$ be a sink strongly connected component. The subspace $W = \mathrm{span}\{e_i : i \in S\}$ is invariant under every $E_{ji} \in \mathcal{S}^*(G)$ (no arrows leave a sink SCC). Thus $\mathcal{A}(G)$ acts reducibly on $\CC^n$. By the double commutant theorem for finite-dimensional $*$-algebras~\cite{takesaki1979theory}, $\mathcal{A}(G) \neq M_n(\CC)$ implies $\mathcal{A}(G)' \neq \CC \cdot I$, so $\commutant_{\mathrm{struct}}(G) \neq \CC \cdot I$.
\end{proof}
...
Consider $n = 2$ with quantum network graph $G$ having coherent edge $\{1,2\}$. The test set is $\mathcal{S}^*(G) = \{E_{12}, E_{21}\}$, the directed support graph $D(G)$ has both arrows, so $D(G)$ is strongly connected and $\defQ^{\mathrm{struct}}(G) = 0$. Explicitly: $\commutant_{\mathrm{struct}}(G) = \{X : [X, E_{12}] = [X, E_{21}] = 0\} = \CC \cdot I$.

\textbf{Non-generic case (aligned operators):} Take $H = \omega \sigma_x$ and a single self-adjoint jump operator $L = \sqrt{\gamma}\, \sigma_x$, where $\sigma_x = E_{12} + E_{21}$. Let $|\pm\rangle = (|1\rangle \pm |2\rangle)/\sqrt{2}$ be the $\sigma_x$-eigenstates. The Lindbladian is:
\[
\Lindblad(\rho) = -i\omega[\sigma_x, \rho] + \gamma(\sigma_x \rho \sigma_x - \rho)
\]
For $\rho$ diagonal in the $\{|+\rangle, |-\rangle\}$ basis: $[\sigma_x, \rho] = 0$ and $\sigma_x \rho \sigma_x = \rho$ (since $\sigma_x^2 = I$). Thus $\Lindblad(\rho) = 0$. Both $|+\rangle\langle +|$ and $|-\rangle\langle -|$ are stationary, giving $\dim(\ker \Lindblad) = 2$ and $\defQ = 1 > 0 = \defQ^{\mathrm{struct}}(G)$.

The commutant of the specific operator $\sigma_x$ is $\commutant(\theta) = \{X : [X, \sigma_x] = 0\} = \mathrm{span}\{I, \sigma_x\}$, which is 2-dimensional---larger than $\commutant_{\mathrm{struct}}(G) = \CC \cdot I$. This is the ``accidental symmetry'': the specific parameter choice $H \propto L$ creates a hidden conservation law.

\textbf{Non-generic locus (algebraic):} The exceptional set is $V_G = \{(\omega, \gamma) : H \in \langle L, L^\dagger, I \rangle_{*\text{-alg}}\}$, i.e., where the Hamiltonian lies in the $*$-algebra generated by the jump operators. For this parametrization with $H = \omega \sigma_x$ and $L = \sqrt{\gamma} \sigma_x$, this is always satisfied---this entire 1-parameter family lies in $V_G$.

\textbf{Restoring genericity:} Perturb to $H' = \omega \sigma_x + \epsilon \sigma_z$ for any $\epsilon \neq 0$. Now $[X, H'] = [X, L] = 0$ requires $[X, \sigma_x] = [X, \sigma_z] = 0$, which forces $X = cI$, and $\defQ = 0$.
\end{example}
...
\]
Then $V_G$ is a proper algebraic subvariety of $\Theta(G)$, characterized by:
\[
V_G = \{\theta : \exists X \notin \commutant_{\mathrm{struct}}(G) \text{ with } [X, H] = [X, L_k] = [X, L_k^\dagger] = 0 \ \forall k\}
\]
In particular, $\theta \in V_G$ iff there exists a ``hidden symmetry''---an operator commuting with the specific generators that does not commute with generic operators of the same support.
\end{proposition}

\begin{proof}
The constraint map $\Phi_\theta(X) = ([X,H], [X,L_1], [X,L_1^\dagger], \ldots)$ has $\ker \Phi_\theta = \commutant(\theta)$. Since $\commutant_{\mathrm{struct}}(G) \subseteq \commutant(\theta)$ always, we have $\defQ(\theta) > \defQ^{\mathrm{struct}}(G)$ iff $\ker \Phi_\theta \supsetneq \commutant_{\mathrm{struct}}(G)$, which occurs iff some $X \notin \commutant_{\mathrm{struct}}(G)$ satisfies $\Phi_\theta(X) = 0$. These are polynomial conditions on $\theta$, defining an algebraic subvariety. It is proper by Theorem~\ref{thm:robust}(b).
\end{proof}
...
In the Q-CRNT framework, this system has:
\begin{itemize}
...
\textbf{DetailedBalance.lean} (7 axioms) --- GNS inner product and spectral theory:
\begin{itemize}
\item \texttt{gnsInnerProduct\_self\_nonneg}: $\langle A, A \rangle_\sigma \geq 0$
\item \texttt{minEigenvalue}: Minimum eigenvalue function
\item \texttt{minEigenvalue\_pos}: $\lambda_{\min}(\sigma) > 0$ for faithful $\sigma$
...
This represents, to our knowledge, the \textbf{first formal verification of Lindblad dynamics in any proof assistant}. Prior quantum formalization efforts~\cite{hietala2021verified,zhou2023coqq} focused on closed systems (unitary evolution) or quantum programs, not open-system master equations.
